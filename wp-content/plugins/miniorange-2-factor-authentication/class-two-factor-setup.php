...
class Two_Factor_Setup{
	...
	function check_mobile_status($tId){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/auth/auth-status';
		$ch = curl_init($url);
	...
	function register_mobile($useremail){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/auth/register-mobile';
		$ch = curl_init($url);
		global $current_user;
	...
	function mo_check_user_already_exist($email){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/admin/users/search';
		$ch = curl_init($url);
	...
	function mo_create_user($currentuser,$email){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/admin/users/create';
		$ch = curl_init($url);
	...
	function mo2f_get_userinfo($email){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/admin/users/get';
		$ch = curl_init($url);
	...
	function mo2f_update_userinfo($email,$authType,$phone,$tname,$enableAdminSecondFactor){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/admin/users/update';
		$ch = curl_init($url);
	...
	function register_kba_details($email,$kba_q_a_list){
		...
		$url = get_site_option('mo2f_host_name') . '/moas/api/auth/register';
		$ch = curl_init($url);
		...
		$size = sizeof($kba_q_a_list);
		$list = "[";
		for($qa = 0; $qa < $size; $qa = $qa + 2){
			$list = $list . "{\"question\":\"" . $kba_q_a_list[$qa] . "\",\"answer\":\"" . $kba_q_a_list[$qa + 1] . "\" },";
		}
		$list = rtrim($list,",");
